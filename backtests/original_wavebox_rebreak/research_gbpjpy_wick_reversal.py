#!/usr/bin/env python3
"""
GBPJPY H1 wick/sweep reversal research.

Hypothesis:
  GBPJPY's long wicks and fast reversals can be traded as failed liquidity
  sweeps, not as ordinary breakouts.
"""
...
OUT_DIR = THIS_DIR / "results_2026_05_26" / "gbpjpy_wick_reversal"
...
SWEEP_BUFFER_FILTERS = [0.05]
WICK_FILTERS = [0.55, 0.65]
CLOSE_LOC_FILTERS = [0.65, 0.75]
RANGE_ATR_FILTERS = [1.00, 1.20]
PREMOVE_FILTERS = [0.8, 1.2, 1.6]
BODY_MAX_FILTERS = [0.65, 0.50]
...
def generate_source(h1: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict] = []
    for i in range(max(max(LOOKBACKS), max(PREMOVE_BARS)) + 1, len(h1) - 1):
        ts = h1.index[i]
                    ...
                    rows.append(
                        {
                            ...
                            "premove_atr": float(row[f"drop_{pre}_atr"]),
                            "h4_align": align,
                            "h4_oppose": oppose,
                            "h4_context": note,
    ...
    if source.empty:
        return source
    source = source.replace([np.inf, -np.inf], np.nan).dropna(
        subset=["wick_ratio", "close_location", "range_atr", "body_ratio", "premove_atr"]
    )
    return source.sort_values(["signal_i", "direction", "lookback", "pre_bars"]).reset_index(drop=True)
...
def score(row: dict) -> float:
    if row["trades"] < 35 or row["is_trades"] < 25 or row["oos_trades"] < 3:
        return -9999.0
    return (
        row["avg_r"] * 90.0
...
def write_report(source: pd.DataFrame, grid: pd.DataFrame, top: pd.DataFrame, chosen: pd.DataFrame) -> None:
    cols = [
        "score",
        "trades",
        "win_rate",
    ...
    lines = [
        ...
        "- TPは固定R",
        "",
        f"source candidates: `{len(source)}`",
        "",
        "## Top Candidates",
        "",
        markdown_table(top[cols], 40),
        "",
        "## Chosen Trades",
        "",
        markdown_table(chosen[trade_cols] if not chosen.empty else chosen, 100),
    ]
...
def main() -> None:
    h1 = prepare_h1()
    source = generate_source(h1)
    source.to_csv(OUT_DIR / "gbpjpy_wick_source_candidates.csv", index=False)
    if source.empty:
    ...
    keys = {
        ...
        "sweep_atr": SWEEP_BUFFER_FILTERS,
        "wick_ratio": WICK_FILTERS,
        "close_location": CLOSE_LOC_FILTERS,
        "range_atr": RANGE_ATR_FILTERS,
        "premove_atr": PREMOVE_FILTERS,
        "body_max": BODY_MAX_FILTERS,
        "h4": H4_FILTERS,
        "rr": RR_LIST,
    ...
    total = math.prod(len(v) for v in keys.values())
    for idx, combo in enumerate(itertools.product(*keys.values()), start=1):
        spec = dict(zip(keys.keys(), combo))
        trades = run_filtered_trades(h1, source, spec)
        row = {**spec, **summarize(trades)}
        row["score"] = score(row)
        grid_rows.append(row)
        if row["score"] > -9999 and (best_row is None or row["score"] > best_row["score"]):
            best_row = row
            best_trades = trades
        if idx % 5000 == 0:
    ...
    grid.to_csv(OUT_DIR / "gbpjpy_wick_grid.csv", index=False)
    top = grid[grid["score"] > -9999].head(80).copy()
    top.to_csv(OUT_DIR / "gbpjpy_wick_top_candidates.csv", index=False)
    best_trades.to_csv(OUT_DIR / "gbpjpy_wick_chosen_trades.csv", index=False)
    write_report(source, grid, top, best_trades)
