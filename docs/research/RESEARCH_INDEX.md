...
2. 早い入口と待った入口を手動マーキングする
3. TradingViewで `Crowd Psychology Simple v0.3` を確認する
4. entry_datetime / entry_price を取れる資料から追加する
5. P03「V字・急反転期待」も20枚並べる
...
|---:|---|---|---|---|---|
| 1 | 受講生エントリー集中パターン研究 | 進行中 | 複数の受講生が同じチャート形状で入る場所を見つけ、成功/失敗の分岐を探す | 匿名化705トレードから、節目抜け、トレンド継続期待、V字、レンジ急変への反応集中を一次抽出 | P01「節目抜け・ブレイク飛び乗り」の画像を20件並べる |
