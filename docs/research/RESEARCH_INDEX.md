...
1. ローカルで抽出した成功候補20件を目視確認する
2. E01「節目抜け後の再確認」とE04「押し戻り継続」に対応する失敗例をペアにする
3. 勝った例にあって、負けた例にない条件を1行ルールにする
4. Pineの緑サインは `抜け後停滞 + 2回目確認` と `押し戻り継続` を優先して強化する
5. E05「転換確認」は単体では緑サインにせず、注意条件として扱う
...
|---:|---|---|---|---|---|
| 1 | トレード心理研究 | 進行中 | 失敗トレードに共通する心理、判断、チャート形状を見つけ、最適エントリーパターンへ変換する | 匿名化、構造化、定量分析、失敗チャート分類、代表例7ケースの可視化、教材化、Pine可視化プロトタイプ、最適エントリー一次抽出まで実施 | 成功候補20件を目視確認し、失敗例と同じ型でペア比較する |
