...
1. TradingView で v0.4 の青帯位置を目視確認する
2. v2.x マトリクス戦略へ「節目飛び乗り抑制」フィルタを試験実装する
3. GBPJPY、XAUUSD、USDJPY で `S/W/C` を各20例ずつ記録する
4. P03「V字・急反転期待」も20枚並べる
5. EURJPY は9件・クラスタ0のため、データ追加後に再分析
...
|---:|---|---|---|---|---|
| 1 | 受講生つまずきクラスタ研究 | 進行中 | 複数人が同じ日時×価格で失敗した場所を特定し、教材化と v2.x フィルタに使う | 137件抽出、29クラスタ（全敗15）、Pine v0.4（赤+青）完成 | v0.4 目視確認 → v2.x 節目飛び乗り抑制フィルタ |
