...
|---:|---|---|---|---|---|
| 1 | トレード心理研究 | 進行中 | 失敗トレードに共通する心理、判断、チャート形状を見つけ、最適エントリーパターンへ変換する | 匿名化、構造化、定量分析、失敗チャート分類、代表例7ケースの可視化、教材化、Pine可視化、最適エントリー一次抽出、成功/失敗ペア比較、GBPJPY H1でのv0.2観察、リアルタイム4段階判定まで実施 | v0.3でFOMO/PANIC/WAITの意味をさらに分け、判定記録表を作る |
