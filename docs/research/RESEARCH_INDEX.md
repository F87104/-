...
| 2026-06-03 | Short Covering Psychology Flow | 売り優勢、売り失敗、ショートカバー、加速を分けて表示するH4可視化を追加。表示は実戦レビューで整理しつつ、検出条件は候補が出る標準感度へ戻した。 | 主要通貨でSHORT COVERING後の最大上昇ATR、継続本数、失敗パターンを記録する |
...
| 2 | Short Covering Psychology Flow | **重要・可視化中** | 売り手の含み益が崩れ、損切り買い戻しに変わる心理遷移を可視化する | H4可視化Pineを実戦レビュー表示へ整理し、標準感度へ再調整済み | SHORT COVERING後の上昇幅、継続本数、失敗例を記録する |
