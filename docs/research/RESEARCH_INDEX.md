...
1. TradingViewでPine v0.2をH4チャートに入れ直す
2. まずC07をOFFにして、C01とC03だけで赤と緑の出方を見る
3. C01とC03が読める量になったら、C07をONにして表示間隔を24から48へ調整する
4. 赤い表示が実際の失敗チャートに近い場所へ出るか確認する
5. 緑の表示が成功エントリー候補として自然に見えるか確認する
...
|---:|---|---|---|---|---|
| 1 | トレード心理研究 | 進行中 | 失敗トレードに共通する心理、判断、チャート形状を見つける | 匿名化、構造化、定量分析、失敗チャート分類、代表例7ケースの可視化、教材化、Pine可視化プロトタイプまで実施 | TradingViewでPineを目視確認し、失敗パターンの逆側が成功候補になるか検証する |
