...
2. 全敗ゾーンごとに「本来待つべき場所」を手動マーキングする
3. v2.x マトリクス戦略へ「節目飛び乗り抑制」フィルタを試験実装する
4. P03「V字・急反転期待」も20枚並べる
5. EURJPY など残り通貨ペアのつまずき可視化を追加する
...
|---:|---|---|---|---|---|
| 1 | 受講生つまずきクラスタ研究 | 進行中 | 複数人が同じ日時×価格で失敗した場所を特定し、教材化と v2.x フィルタに使う | 137件の実エントリー抽出、29クラスタ（全敗15）、GBPJPY/USDJPY/XAUUSD Pine v0.3 可視化、通貨別心理仮説を追加 | 通貨別 `S/W/C` 20例記録と全敗ゾーンの「待つ場所」マーキング |
