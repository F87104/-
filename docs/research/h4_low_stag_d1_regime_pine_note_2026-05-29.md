...
`D1 RSI35-55 + H4厳選 + 60本lookback厳格化`:

- 条件: `lookback != 60 or (support_age <= 24 and break_depth >= 0.25ATR)`
- 11 trades
- 勝率 100.00%
- 総R +21.24R
- PF inf
...
この数字は強すぎるため、そのまま過信しない。狙いは「短期lookbackのだましだけを削る」ことで、Pine照合とフォワード監視の本線候補として扱う。

## 実装上の注意

- D1 RSIは未確定日足を使わない。Pineでは `request.security(..., "D", ta.rsi(close, 14)[1])`。
- このPineはH4専用。H1で表示すると120本が1か月ではなく約5日になり、Python検証と別物になるため、デフォルトでH4以外は停止する。
- Pythonの practical 集合は複数lookbackを同一時刻で重複除去している。Pineでは優先順で最初に通ったlookbackを選ぶ。
- 60本lookback厳格化は、優先順で60本候補を選んだ後に適用する。60本候補を除外してから後順位lookbackへ乗り換えるとPython検証とズレるため禁止。
- Pythonは次足始値で約定し、その始値から2Rターゲットを計算する。Pineは `calc_on_order_fills=true` で約定後に `strategy.position_avg_price` からTPを再計算する形へ寄せる。
...
- 詳細なPine一致監査は `docs/research/h4_low_stag_pine_parity_audit_2026-05-29.md` を参照。
