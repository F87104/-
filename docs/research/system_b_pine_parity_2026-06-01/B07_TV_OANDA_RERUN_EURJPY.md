# B07 EURJPY — TV OANDA データでの Python 再実行
...
|------|------|
| F87104 export（従来） | **3** |
| TV OANDA H4 CSV（今回） | **4** |
| 同一 signal_time | **0** |
| TVテスターと±2h（entry_time_tv） | **0/4** |
...
- `python_expected_b07_tv_oanda_eurjpy.csv`
- `b07_f87104_vs_tv_oanda_eurjpy.csv`
- `b07_tv_oanda_vs_tester_eurjpy.csv`（テスターCSVがある場合）
