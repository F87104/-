...
### 6.1.1 4段階プリセット (2026-05-30 追加)

Pine 実装ではユーザーが厳しさを 1 入力で切り替えられるよう、4 段階プリセット + Manual を提供する。

| パラメータ | Strict | Balanced | **Loose ★既定** | Very Loose |
|---|---|---|---|---|
| `minDropAtr` | 3.5 | 3.0 | **2.5** | 2.0 |
| `maxRecoveryRatio` | 1.0 | 1.5 | **2.0** | 3.0 |
| `minSpeedRatio` | 1.20 | 1.10 | **1.00** | 0.80 |
| `crossBufferAtr` | 0.05 | 0.03 | **0.00** | 0.00 |
| `maxWatchBars` | 36 | 48 | **72** | 120 |
| `minBodyRatio` | 0.60 | 0.55 | **0.50** | 0.40 |
| `minCloseLocation` | 0.60 | 0.55 | **0.50** | 0.40 |
| `htfRsiAvoid` | 35 | 32 | **28** | 20 |
| `vNearEmaAtr` | 1.0 | 1.5 | **2.5** | 4.0 |
| `lowVolMin` | 0.6 | 0.5 | **0.4** | 0.3 |
| `nearHighMinR` | 1.5 | 1.2 | **1.0** | 0.5 |
| 期待トレード数/年/通貨 | **1〜3** | **4〜8** | **10〜20** | **20〜40** |
| 期待勝率 | 60〜65% | 55〜62% | 48〜55% | 42〜50% |
| 期待 PF | 1.8〜2.5 | 1.6〜2.2 | 1.3〜1.8 | 1.0〜1.4 |

#### 既定を Loose にした理由

初版 Strict で 13 年 GBPJPY バックテストにおいて **エントリー数 1 件** という極端な結果が発生。
複数フィルタの AND 結合で確率が掛け算で潰れ、実運用に堪える頻度が出なかったため、
**まず Loose で母集団を確認 → 必要に応じて Balanced/Strict に上げる** ワークフローを推奨。

#### さらに増やしたい場合

| 操作 | 増加率 |
|---|---|
| `useNearHighFilter = false` | +30〜50% |
| `useNearEma = false` | +50% |
| `useTrendFilter = false` | +100% 以上（ただし勝率低下） |

最大化すれば 30〜60 トレード/年/通貨だが、勝率は 40〜50% 程度に落ちる前提。
