"""
検証結果一覧 → Google スプレッドシート用 CSV 生成
=================================================

出力先: docs/spreadsheet/

シート構成:
  01_overall_judgment.csv    : 採用判定マトリクス (本番運用に使うか)
  02_method_catalog.csv      : 全検証ディレクトリ一覧 + 結論
  03_by_symbol.csv           : 通貨別成績マトリクス
  04_by_year.csv             : 年別成績
  05_is_oos.csv              : IS/OOS 期間比較
  06_anti_patterns.csv       : やったけどダメだった一覧
  07_tf_compare.csv          : 時間軸比較
  08_research_sub.csv        : 補助研究 (pyramiding, fakeout, etc.)
  99_glossary.csv            : 用語集

Google スプレッドシートでの使い方:
  1. https://sheets.new で空のスプレッドシートを開く
  2. 「ファイル → インポート → アップロード」
  3. 各CSVを別シートとしてインポート
  4. シート名は CSV のファイル名と同じになる
"""
...
# 01. 採用判定マトリクス
# ===================================================================
def sheet_01_judgment():
    headers = [
        "No", "手法名", "期間", "TF", "通貨数",
        "Trades", "WR%", "PF", "Total_R", "Max_DD_R",
        "連敗", "判定", "コメント"
    ]
    rows = [
        [1, "TrendBreakV1 HYBRID (baseline)", "2015-2024", "H1", "7", 461, 36.88, 1.62, 191.53, 14.02, 13, "採用(主力)", "fakeout filter なし"],
        ["1'", "TrendBreakV1 HYBRID (AUDJPY除外)", "2015-2024", "H1", "6", 381, 39.37, 1.79, 194.61, 11.94, 11, "採用", "推奨6通貨"],
        [2, "H4 T5+MACD+BB 実戦用", "2015-2024", "H4", "7", 34, 61.76, 3.56, 29.20, 4.35, 5, "採用(補助)", "Strict 0.75-1.00+REC1.2"],
        ["2'", "H4 T5+MACD+BB 実戦用", "2025-2026 OOS", "H4", "7", 15, 66.67, 2.44, 7.26, 2.02, 2, "OOS再現", "過剰最適化なし"],
        [3, "TrendBreakV1 + T5 両方フル (採用案)", "2015-2024", "H1+H4", "6", 411, 40.88, 1.86, 219.94, 11.94, 11, "本番運用", "推奨"],
        [4, "Sai Best Method (急な揺り戻し)", "2015-2024", "H1", "7", 378, 44.4, 1.47, 98.84, 13.21, 9, "archive", "v1で採用、v2で外す"],
        [5, "Sai H1 全体 (4手法込み)", "2015-2024", "H1", "7", 1108, 41.6, 1.05, 99.91, 21.9, "?", "不採用", "平均PF低い"],
        [6, "H4 急落V字 単独 (strict)", "2015-2024", "H4", "7", 39, 30.77, 0.72, -7.16, 13.20, 5, "単独NG", "環境認識用"],
        [7, "Synapse v0 (エリオット風 H4)", "2015-2024", "H4", "7", 413, 31.96, 0.85, -42.41, 48.82, 19, "不採用", "機械化困難"],
        [8, "Synapse v1 (改善版 H1)", "2015-2024", "H1", "7", 81, 56.79, 1.42, 14.78, 5.39, 5, "研究中", "A_earlyモード有望"],
        [9, "Elliott W5 RR3 LOOSE", "2015-2024", "H1", "7", 450, 30.0, 1.23, 73.33, 34.70, 17, "不採用", "DD大"],
        [10, "VFIB_618_BODY60_LONG_RR2", "2015-2024", "H1", "7", 1513, 38.60, 1.19, 186.64, 50.87, 16, "不採用", "件数◎ 平均R薄"],
        [11, "TrendBreakV1 (NAS100)", "2015-2024", "H1", "1", "?", "?", "?", "?", "?", "?", "参考", "インデックス検証"],
        [12, "TrendBreakV1 (SPX500)", "2015-2024", "H1", "1", "?", "?", "?", "?", "?", "?", "参考", "インデックス検証"],
        [13, "TrendBreakV1 Pyramiding sweep", "2015-2024", "H1", "7", "各種", "-", "-", "-", "-", "-", "研究", "1エントリーが最適"],
        [14, "TrendBreakV1 騙し回避フィルタ", "2015-2024", "H1", "7", "各種", "-", "-", "-", "-", "-", "研究", "body60+early1 が候補"],
    ]
    write_csv("01_overall_judgment.csv", headers, rows)


# ===================================================================
# 02. 手法別カタログ (全検証ディレクトリ)
# ===================================================================
def sheet_02_catalog():
    headers = ["カテゴリ", "検証名", "パス", "概要", "結論"]
    rows = [
        # TrendBreakV1 系
    ...
    write_csv("02_method_catalog.csv", headers, rows)


# ===================================================================
# 03. 通貨別成績マトリクス
# ===================================================================
def sheet_03_by_symbol():
    headers = [
        "通貨", "TB単独_Trades", "TB単独_WR", "TB単独_PF", "TB単独_Total_R", "TB単独_DD",
        "T5実戦_Trades", "T5実戦_WR", "T5実戦_PF", "T5実戦_Total_R", "T5実戦_DD",
        "両方フル_Trades", "両方フル_WR", "両方フル_PF", "両方フル_Total_R", "両方フル_DD", "両方フル_連敗",
        "採用判定"
    ]
    rows = [
        ["XAUUSD", 75, 44.00, 2.21, 53.20, 6.19, 7, 71.43, 7.71, 8.64, 1.01, 79, 45.57, 2.29, 58.15, 6.19, 6, "採用 エース"],
        ["USDJPY", 64, 37.50, 1.72, 29.91, 9.37, 8, 62.50, 4.89, 7.92, 1.01, 68, 38.24, 1.74, 31.88, 9.37, 9, "採用"],
        ["EURJPY", 55, 30.91, 1.29, 11.42, 9.39, 2, 50.00, 1.96, 0.97, 0.00, 57, 31.58, 1.31, 12.39, 7.41, 7, "採用 (弱め)"],
        ["GBPJPY", 59, 42.37, 2.11, 39.11, 6.16, 10, 60.00, 2.60, 6.47, 2.02, 64, 42.19, 2.05, 40.08, 6.16, 6, "採用"],
        ["CHFJPY", 64, 35.94, 1.56, 24.35, 9.37, 5, 80.00, 7.90, 6.93, 1.00, 67, 37.31, 1.62, 27.29, 9.37, 10, "採用"],
        ["SILVER", 64, 43.75, 1.86, 36.62, 6.41, 3, 66.67, 2.06, 1.12, 0.00, 64, 43.75, 1.87, 36.62, 6.41, 5, "採用"],
        ["AUDJPY", 80, 25.00, 0.95, -3.08, 17.95, 4, 75.00, 4.83, 3.86, 0.00, 82, 25.61, 0.97, -2.09, 17.95, 12, "❌ 除外"],
    ]
    write_csv("03_by_symbol.csv", headers, rows)


# ===================================================================
# 04. 年別成績 (TrendBreakV1 単独 baseline)
# ===================================================================
def sheet_04_by_year():
    headers = ["年", "Trades", "WR%", "PF", "Total_R", "DD_R", "連敗", "コメント"]
    rows = [
    ...
    write_csv("04_by_year.csv", headers, rows)
...
# ===================================================================
def sheet_05_is_oos():
    headers = ["手法", "プリセット", "期間", "Trades", "WR%", "PF", "Total_R", "DD_R", "連敗"]
    rows = [
        ["H4 T5 MACD BB", "Strict 0.75-1.00 + REC1.2", "IS 2015-2024", 99, 55.6, 2.55, 59.8, 6.7, 6],
        ["H4 T5 MACD BB", "Strict 0.75-1.00 + REC1.2", "OOS 2025-2026", 15, 66.7, 2.44, 7.3, 2.0, 2],
        ["H4 T5 MACD BB", "Robust 0.75-1.05 + REC1.5", "IS 2015-2024", 129, 52.7, 2.17, 64.5, 8.8, 10],
        ["H4 T5 MACD BB", "Robust 0.75-1.05 + REC1.5", "OOS 2025-2026", 21, 61.9, 2.27, 9.2, 2.0, 2],
        ["H4 T5 MACD BB", "Current 0.60-1.10 + REC1.0", "IS 2015-2024", 142, 50.7, 1.81, 53.9, 9.8, 9],
        ["H4 T5 MACD BB", "Current 0.60-1.10 + REC1.0", "OOS 2025-2026", 24, 54.2, 1.42, 4.7, 2.3, 2],
        ["Synapse v1", "USDJPY", "IS 2015-2024", 8, 75.0, 7.07, 6.5, 1.02, 1],
        ["Synapse v1", "CHFJPY", "IS 2015-2024", 8, 75.0, 3.27, 4.8, 2.10, 2],
        ["Synapse v1", "GBPJPY", "IS 2015-2024", 11, 63.6, 2.00, 4.1, 3.08, 3],
    ]
    write_csv("05_is_oos.csv", headers, rows)
...
def sheet_06_anti():
    ...
    write_csv("06_anti_patterns.csv", headers, rows)


# ===================================================================
# 07. 時間軸比較 (V字回復系)
# ===================================================================
def sheet_07_tf_compare():
    headers = ["TF", "戦略", "Trades", "WR%", "PF", "Total_R", "DD_R", "採用判定"]
    rows = [
        ["H1", "VFIB_618_BODY60_LONG_RR2", 1513, 38.6, 1.19, 186.64, 50.87, "❌ DD大"],
        ["H4", "VFIB_618_BODY60_LONG_RR2", 566, 43.29, 1.33, 103.78, 14.62, "⚠️"],
    ...
    write_csv("07_tf_compare.csv", headers, rows)


# ===================================================================
# 08. 補助研究 (Pyramiding / Fakeout / Cost)
# ===================================================================
def sheet_08_research_sub():
    headers = ["カテゴリ", "ルール / 設定", "Trades", "WR%", "PF", "Total_R", "DD_R", "コメント"]
    rows = [
        ...
        ["Cost", "コストなし", "-", "-", "-", 184, "-", "audit前"],
        ["Cost", "スプレッド+滑り込み", "-", "-", "-", 146, "-", "-21% でも維持"],
        ["Relaxation", "HYBRID最適化", "-", "-", "-", "+23%", "-", "頻度+27%"],
    ]
    write_csv("08_research_sub.csv", headers, rows)


# ===================================================================
# 99. 用語集
# ===================================================================
def sheet_99_glossary():
    headers = ["用語", "意味"]
    rows = [
        ...
        ["HYBRID", "TrendBreakV1 の通貨別最適化パラメータ集"],
        ["REC1.2", "V字回復速度: 下落本数の 1.20 倍以内に 61.8-80% 回復"],
    ...
    write_csv("99_glossary.csv", headers, rows)


def build_combined_tsv():
    """全シートを1つのTSVにまとめる (コピペ用)。"""
    files = sorted(p for p in OUT.glob("*.csv") if p.name != "ALL_SHEETS.tsv")
    lines = []
    for csv_path in files:
        ...
        lines.append("")
        lines.append("")
    out = OUT / "ALL_SHEETS.tsv"
    out.write_text("\n".join(lines), encoding="utf-8")
    print(f"  ✓ ALL_SHEETS.tsv (全シート結合)")


def main():
    print("=" * 60)
    print("Backtest Index → Google Spreadsheet CSV Builder")
    print("=" * 60)
    sheet_01_judgment()
    sheet_02_catalog()
    sheet_03_by_symbol()
    ...
    sheet_06_anti()
    sheet_07_tf_compare()
    sheet_08_research_sub()
    sheet_99_glossary()
    build_combined_tsv()
    print(f"\n✅ 出力先: {OUT}")
    print(f"\nGoogle スプレッドシートで開くには:")
    print("  方法A: 各CSVを別シートとしてインポート")
    print("    1. https://sheets.new で空のスプレッドシートを開く")
    print("    2. 「ファイル → インポート → アップロード」")
    print("    3. 各CSVを別シートとしてインポート")
    print("  方法B: ALL_SHEETS.tsv を開いて全部コピペ (各セクションごとに分割)")
