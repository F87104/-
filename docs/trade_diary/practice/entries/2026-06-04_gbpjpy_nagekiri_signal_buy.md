...
| 決済 | **214.110**（OCO逆指・損切り） |
| 確定損益 | **約 -115,200 円**（建値差 -0.576 / 約 -58 pips） |
...
- 6/3 ゴールドとは対照的に、**SL は発注済み** → 想定内の損切りで終了（手仕舞いの遅れ・大損拡大は回避）。
...
| 4 | 入口はダメでも、**出口のルール遵守**で -11.5万で止まった（6/3 の -66.8万型は回避） |
