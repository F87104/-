...
## 気づき — ルール通りに維持（2026-06-06）

4300付近から4345へ戻り、**早期利確 vs OCO維持** で迷ったが、**OCO そのまま維持** を選択。
...
| 理由 | エントリー時に決めた出口。保有中に相場観で書き換えない |

### 気づき（3点）

1. **「上がってきた」は主観だった**  
   4300→4345 の戻りは事実だが、「危ない・利確すべき」は解釈。逆指 4408 未触れ・指値 4236 未到達なら、ルール上は **何もしない** が正解。

2. **結果を出すにはルール通り**  
   6/3（ルールなし→大損）と GBPJPY（出口OCO→想定内損切り）の対比。迷ったときほど **事前に決めた1本のルール** に戻る。

3. **6/3と同型を避けた**  
   6/3は「戻るはず・まだ行く」でルール上書き。**今回の「戻ってきたから怖い」も、ルール外の感情介入になり得る** — と自覚して手を出さなかった。

### 次回から（出口ルール1行）

エントリー時に必ず書く:

> **出口ルール: V1 OCO のみ** ／ **V1 + E04 併用**

保有中はその1行だけ見る。

### 検証（2026-06-08）

4345付近の戻り後、**再度下落**。早期利確せず **V1 OCO 維持** は正解だった。

| 項目 | 内容 |
|---|---|
| 迷いのとき | 「上がってきた」→ 主観と自覚 → 手を出さない |
| その後 | 下落再開 → **ルール通りが機能** |
| 教訓 | 保有中の解釈より、**エントリー時に決めた OCO** を優先 |

> 6/3は「まだ行く」でルール上書き。**今回は「怖い」で上書きしなかった** — 対称の勝ち。
