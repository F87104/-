...
| 方向 | 買い |
| 数量 | 20（20万通貨） |
| 建値 | **201.368**（シグナル点灯エントリー） |
| エントリー理由 | **本命v2.1 SQZ シグナル点灯エントリー** |
...
| 項目 | 価格 |
|---|---:|
| シグナル (SQZ ラベル) | 201.319 |
| **建値（約定）** | **201.368** |
| Stop Loss (SL) | 200.251 |
| Take Profit (TP) | 203.456 |
| リスク幅（建値基準） | 1.117（約 111.7 pips） |
| リスクリワード（建値→TP） | 約 1.87R |
| 構造 | SQZ（踏み上げ棚ブレイク） |
...
- 6 月初旬 204 付近から 200.3 付近までの急落後、下値で棚形成 → 棚高値上抜け。**シグナル点灯 @201.368 でエントリー**（Pine ラベル Entry 201.319 は確定足 close 基準）。
