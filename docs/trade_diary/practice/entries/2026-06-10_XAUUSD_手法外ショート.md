...
### 1. V1 利確直後の再参入（同方向・手法外）

| 項目 | 内容 |
|---|---|
| 直前トレード | E-2026-06-04-002（V1 正式）@ 4368.40 → 4200 利確 **+1,091,277 円** |
| 本トレード | 約 10 時間後、**4164.77 で再ショート 50.0** |
| 問題 | 正式シグナルなしで同方向に再エントリー。数量も V1（40）より **50 に増加** |
| 心理 | 「4200 下もまだ落ちる」という **勝ち後の欲** がエントリーを後押しした可能性 |

### 2. 4100 割れ確認前のエントリー

| 項目 | 内容 |
|---|---|
| 分析 | 日足 4100 下抜け → 加速シナリオ（3359 真空地帯） |
| 行動 | 建値 **4164.77** = 4100 **上** 60 ドル付近でエントリー |
| 問題 | **分析と行動の不一致** — 4100 日足終値確認を待たずに建玉 |
| 教訓 | 仮説は「4100 割れ後の加速」。割れる前は **待つ** べきだった |

### 3. 6/3 教訓の部分的な適用

| 6/3（E-2026-06-03-001） | 本トレード |
|---|---|
| 投げ切り買い、SL 未発注 → **-668,492** | OCO（SL 4236 / TP 4000）**発注済み** ✅ |
| ルールなし裁量 | 依然 **ルールなし裁量** ❌ |
| 評価 | SL/TP は改善。**エントリー根拠のルール化** は未達 |

### 4. V1 成功が裁量再参入を正当化していない

- V1 で +109 万円を取った ≠ 次のトレードも勝てる
- 正式シグナル外の再参入は **運用ルール違反** に近い
- 「勝っているからもう一発」は **連勝バイアス / 過信** の典型

### 5. 良かった点

| 項目 | 評価 |
|---|---|
| OCO 設定（SL 4236 / TP 4000） | ✅ 6/3 教訓を SL 面で反映 |
| 手法外ラベル付け | ✅ 日誌上で正式シグナルと分離 |
| D1 4100 シナリオ分析 | ✅ 仮説・無効化条件・価格マップを文書化 |
| 実スクショ注釈 | ✅ TradingView 生画像ベースで記録 |

### 6. 建玉中の運用ルール（再確認）

```
【維持】OCO（SL 4236 / TP 4000）を維持。手動決済しない
【禁止】増し玉・ナンピン・4100 割れ前の追加ショート
【確認】日足終値 < 4100 → 加速シナリオ有効 → TP 4000 待ち
【撤退】4236 タッチ or 日足 2 本連続 4200 上 → 戻りシナリオ（OCO 任せ）
```

### 7. 一行まとめ

> **V1 利確後の欲で手法外再ショート。OCO は良いが、4100 確認前のエントリーと数量増は再発防止が必要。**
